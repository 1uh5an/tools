...
var domain_ip chan string
var nocdnip chan string
...
func flagInit() {
	...
	flag.StringVar(&resolvers, "r", "", "DNS服务器")
	...
	if resolvers != "" {
		...
		if fileutil.FileExists(resolvers) {
			...
			for _, rr := range strings.Split(resolvers, ",") {
				dnsxOptions.BaseResolvers = append(dnsxOptions.BaseResolvers, prepareResolver(rr))
			}
		}
	}
...
func readfile(filename string, inputchan chan<- string) {
	defer wg.Done()
...
func checkCDN(inputchan <-chan string, nocdnip chan<- string, domain_ip chan<- string, exitchan chan<- bool) {
	defer wg.Done()
	for domain := range inputchan {
		...
		// 先走本地CDN CNAME检查
		if found := CheckCNAME(domain); found {
		...
		domain_ip <- fmt.Sprintf("%s ==> %v", domain, ips)
		if len(ips) > 1 {
			continue
		}
		...
		for _, ip := range ips {
			...
			if found, provider, err := client.Check(netip); found && err == nil {
				log.Println(fmt.Sprintf("%s ==> %s is part of %s cdn", domain, ip, provider))
			} else {
				// 2. ip cidr
				if found := CheckCIDR(netip); found {
					log.Println(fmt.Sprintf("%s ==> %s has cidr cdn", domain, ip))
				} else {
					// 3. ip asn
					lock.Lock()
					found := CheckASN(netip)
					lock.Unlock()
					if found {
						log.Println(fmt.Sprintf("%s ==> %s has asn cdn", domain, ip))
					} else {
						log.Println(fmt.Sprintf("%s ==> %s has no cdn", domain, ip))
						nocdnip <- ip
					}
				}
			}
...
func main() {
	...
	domain_ip = make(chan string, 1000)
	nocdnip = make(chan string, 1000)
	exitchan = make(chan bool, threads)
	client, err = cdncheck.NewWithCache()
	wg.Add(threads)
	wg.Add(4)
	go readfile(target, inputchan)
	for i := 0; i < threads; i++ {
		go checkCDN(inputchan, nocdnip, domain_ip, exitchan)
	}
	go func() {
		defer wg.Done()
		for i := 0; i < threads; i++ {
